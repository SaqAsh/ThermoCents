...
def strip_date_components(
    df: pd.DataFrame, col_name: str, granularity_level: Literal["month", "day"]
) -> pd.DataFrame:
    ...
    Args:
        ...
        granularity_level (str): Level of granularity - options: "month" or "day".
    ...
    # Based on the granularity level transform the column accordingly
...
def sort_and_truncate_to_min_date_range(
    df1: pd.DataFrame,
    date_col1_name: str,
    df2: pd.DataFrame,
    date_col2_name: str,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Truncate two DataFrames to share the same latest date by removing rows with
    dates beyond the minimum maximum date.
    ...
    Returns:
        tuple[pd.DataFrame, pd.DataFrame]: A tuple containing both DataFrames truncated
            to the same date range, where the ending date is the minimum of the two
            maximum dates.
    """
